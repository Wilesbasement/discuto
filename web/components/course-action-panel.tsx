...
export function CourseActionPanel({ courseId, courseName, holes }: CourseActionPanelProps) {
  ...
  async function requireUser() {
    ...
    if (error || !data.user) throw new Error("Log in first. Then you can save, check in, and post scores.");
    return data.user;
  }
  ...
  async function postRound(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!Number.isFinite(totalScore)) throw new Error("Enter a valid score.");
    ...
    <section className="panel page-stack">
      <div className="post-card-top">
        <div>
          ...
          <h2>Make this course live</h2>
          <p className="dashboard-copy">Save it, check in, post a score, and give other players a reason to return.</p>
        </div>
      ...
      <div className="dashboard-grid">
        <form className="panel page-stack" onSubmit={checkIn}>
          <h3>Check in</h3>
          <label>
            Notes
            <textarea className="form-input" name="notes" placeholder="Course is dry, league night starting soon, wind on the back nine..." />
          </label>
          <label>
            Optional score
            <input className="form-input" name="score" type="number" placeholder="54" />
          </label>
          <button className="button button-primary" disabled={busy === "checkin"}>
            {busy === "checkin" ? "Checking in..." : "Check in"}
          </button>
        </form>

        <form className="panel page-stack" onSubmit={postRound}>
          <h3>Post score</h3>
          <label>
            Total score
          ...
          <button className="button button-primary" disabled={busy === "round"}>
